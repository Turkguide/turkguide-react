...
/**
 * profiles.accepted_terms_at tek doğruluk kaynağı; state ve RLS/0-satır hatalarında yeniden yükler.
...
export async function hydrateTermsAcceptanceFromDb({ supabase, user, setUser }) {
  ...
  if (error) {
    ...
    if (import.meta.env.DEV)
      console.warn(DBG, "query failed; using frontend effective only", error.message, { ok });
  ...
  if (import.meta.env.DEV) {
    console.warn(DBG, "blocked — no DB acceptance", { userId, created_at: data?.created_at });
  }
